...
export default function EventDetalis() {
  ...
  const [showDetails, setShowDetails] = useState(false);
    const{Host,adminToken,formatDate}=useContext(MyContext)
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${Host}/api/admin/scoreDetails/${id}`,{
            headers:{
                token:adminToken
            }
        });
  ...
  return (
    <div className="score-container">
      <div className='text-center'>
        ...
        <hr />
        <p> Team Name :{scoreData.team_id.name} </p>
      ...
      <button className='btn btn-primary' onClick={() => setShowDetails(!showDetails)}>
        {showDetails ? 'Hide Answers' : 'Show Answers'}
      </button>
    <hr />
      </div>
      {showDetails && (
